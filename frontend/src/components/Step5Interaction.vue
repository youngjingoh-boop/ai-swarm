        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                ...
                <div class="tools-card-subtitle">A fast-chat version of the Report Agent — it can call 4 specialized tools and holds AI Swarm's full memory</div>
              </div>
              ...
              <div class="tools-grid">
                <div class="tool-item tool-purple">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">Aligns real-world seed data with simulated environment state, combining Global/Local Memory to deliver deep cross-time attribution analysis</div>
                  </div>
                </div>
                <div class="tool-item tool-blue">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">PanoramaSearch Panoramic Trace</div>
                ...
                <div class="tool-item tool-orange">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">A GraphRAG-based instant query interface with optimized indexing, for quickly pulling specific node attributes and discrete facts</div>
                  </div>
                </div>
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">Autonomous interviewing that holds multi-round conversations with individuals in the simulated world in parallel, gathering unstructured opinions and psychological state</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? 'Chat with the Report Agent to dig deeper into the report' : 'Chat with a simulated agent to hear their perspective' }}
              </p>
            </div>
            <div 
          ...
          <div class="chat-input-area">
            <textarea 
              v-model="chatInput"
              class="chat-input"
              placeholder="Ask the Report Agent anything…"
...
const chatHistoryCache = ref({}) // Caches all chat histories: { 'report_agent': [], 'agent_0': [], 'agent_1': [], ... }
...
const renderMarkdown = (content) => {
  ...
  // Clean up <p><br> immediately before a block element (caused by extra blank lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Collapse consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean up <br> right before a paragraph/div start tag following a block element
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-contiguous ordered lists: keep numbers incrementing when a single-item <ol> is split by paragraph content
...
const sendMessage = async () => {
  ...
  try {
    ...
    scrollToBottom()
    // Automatically save the chat history to the cache
    saveChatHistory()
  }
}
...
const sendToAgent = async (message) => {
  ...
  if (res.success && res.data) {
    ...
    // Convert the object dict to a value, preferring the reddit platform's reply
    let responseContent = null
    const agentId = selectedAgentIndex.value

    if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
      // Prefer the reddit platform reply, then twitter
...
const submitSurvey = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Convert the object dict to an array
...
.generated-content :deep(.md-h2) { font-size: 20px; border-bottom: 1px solid var(--border-light); padding-bottom: 8px; }
...
.report-agent-tools-card {
  border-bottom: 1px solid var(--border);
  background: linear-gradient(135deg, var(--surface-alt) 0%, var(--surface-alt) 100%);
}
