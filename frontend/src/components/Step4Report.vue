...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // 提取统计数据 - 匹配"Related predicted facts: X"格式
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/\*\*Agents Interviewed:\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    // 提取采访摘要
    const summaryMatch = text.match(/### Interview Summary & Key Takeaways\n([\s\S]*?)$/)
    if (summaryMatch) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Sub-questions generated for drift queries'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '（No response from this platform）' || t === '（该平台未获得回复）' || t === '(该平台未获得回复)' || t === '[无回复]'
    }
    ...
    return () => h('div', { class: 'interview-display' }, [
      ...
      props.result.interviews.length > 0 && h('div', { class: 'interview-detail' }, [
        ...
        props.result.interviews[activeIndex.value]?.selectionReason && h('div', { class: 'selection-reason' }, [
          h('div', { class: 'reason-label' }, 'Why this agent was selected'),
          h('div', { class: 'reason-content' }, props.result.interviews[activeIndex.value].selectionReason)
        ]),

        // Q&A Conversation Thread - one question per answer
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle (only shown when there are real dual-platform answers)
